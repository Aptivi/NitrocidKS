    ...
    Sub Linotypo_DoWork(sender As Object, e As DoWorkEventArgs) Handles Linotypo.DoWork
        ...
        Try
            ...
            If LinotypoTextColumns <= 0 Then
                ...
                WdbgConditional(ScreensaverDebug, "I", "{0} columns is impossible to print. Setting to 1...", LinotypoTextColumns)
            ...
            ElseIf LinotypoTextColumns > 3 Then
                ...
                WdbgConditional(ScreensaverDebug, "I", "Setting to 3 to make small terminals happy...")
                LinotypoTextColumns = 3
            End If
            If LinotypoEtaoinThreshold <= 0 Then
                'We can't have a negative threshold!
                WdbgConditional(ScreensaverDebug, "I", "{0} characters is impossible to handle. Setting to 1...", LinotypoEtaoinThreshold)
            ...
            ElseIf LinotypoEtaoinThreshold > 8 Then
                ...
                WdbgConditional(ScreensaverDebug, "I", "Setting to 8...")
            ...
            WdbgConditional(ScreensaverDebug, "I", "Minimum speed from {0} WPM: {1} CPM", LinotypoWritingSpeedMin, CpmSpeedMin)
            WdbgConditional(ScreensaverDebug, "I", "Maximum speed from {0} WPM: {1} CPM", LinotypoWritingSpeedMax, CpmSpeedMax)
            WdbgConditional(ScreensaverDebug, "I", "Maximum characters: {0} (satisfying {1} columns)", MaxCharacters, LinotypoTextColumns)
            WdbgConditional(ScreensaverDebug, "I", "Width threshold: {0}", ColumnRowConsoleThreshold)
            ...
            Do While True
                ...
                If Linotypo.CancellationPending = True Then
                    ...
                    Wdbg(DebugLevel.I, "All clean. Linotypo screensaver stopped.")
                    SaverAutoReset.Set()
                    Exit Do
                Else
                    'Variables
                    ...
                    If TryParsePath(LinotypoWrite) AndAlso File.Exists(LinotypoWrite) Then
                        ...
                        WdbgConditional(ScreensaverDebug, "I", "Opening file {0} to write...", LinotypoWrite)
                    ...
                    For Each Paragraph As String In LinotypeWrite.SplitNewLines
                        ...
                        If CurrentColumn = 1 Then
                            WdbgConditional(ScreensaverDebug, "I", "Column 1. Printing newline...")
                            Console.WriteLine()
                        Else
                            WdbgConditional(ScreensaverDebug, "I", "Column {0}. Setting left to {1}...", CurrentColumn, CurrentColumnRowConsole)
                        ...
                        For IncompleteSentenceIndex As Integer = 0 To IncompleteSentences.Count - 1
                            ...
                            For StruckCharIndex As Integer = 0 To IncompleteSentence.Length - 1
                                ...
                                If Console.CursorTop = 0 Then
                                    WdbgConditional(ScreensaverDebug, "I", "Line repeat in first line in new column. Indenting...")
                                    If CurrentColumn = 1 Then
                                        WdbgConditional(ScreensaverDebug, "I", "Column 1. Printing newline...")
                                        Console.WriteLine()
                                    Else
                                ...
                                If EtaoinMode Then
                                    ...
                                    Select Case LinotypoEtaoinType
                                        Case FillType.EtaoinComplete, FillType.EtaoinPattern
                                            ...
                                            If LinotypeKeyIndex = 7 Then
                                                WdbgConditional(ScreensaverDebug, "I", "Key index exceeded 7. Advancing to next column...")
                                                LinotypeKeyIndex = 0
                                                LinotypeColumnIndex += 1
                                            End If
                                            If LinotypeColumnIndex = LinotypeMaxColumnIndex + 1 Then
                                                WdbgConditional(ScreensaverDebug, "I", "Column indexes exceeded. Resetting...")
                                    ...
                                    End Select
                                    WdbgConditional(ScreensaverDebug, "I", "Key index: {0} | Column index: {1}", LinotypeKeyIndex, LinotypeColumnIndex)
                                Else
                                    ...
                                    If LinotypoGuaranteed Then
                                        ...
                                        WdbgConditional(ScreensaverDebug, "I", "Capped Etaoin: {0}", CappedEtaoin)
                                ...
                                If CountingCharacters Then
                                    WdbgConditional(ScreensaverDebug, "I", "Counter increased. {0}", CharacterCounter)
                                    CharacterCounter += 1
                                    If CharacterCounter > LinotypoEtaoinThreshold Then
                                        ...
                                        WdbgConditional(ScreensaverDebug, "I", "Etaoin mode on because threshold reached.")
                                        EtaoinMode = True
                                ...
                                If EtaoinMode And (StruckCharIndex = MaxCharacters - 1 Or StruckCharIndex = IncompleteSentence.Length - 1) Then
                                    WdbgConditional(ScreensaverDebug, "I", "Etaoin mode off because end of sentence.")
                                    StruckCharIndex = -1
                                    EtaoinMode = False
                            ...
                            If CountingCharacters Then
                                WdbgConditional(ScreensaverDebug, "I", "Sentence ended before Etaoin mode is activated. Resetting counter...")
                                CountingCharacters = False
                                CharacterCounter = 0
        ...
        If LinotypoTextColumns > 1 Then
            If Console.CursorTop >= Console.WindowHeight - 2 Then
                ...
                If CurrentColumn >= LinotypoTextColumns Then
                    '...wait until retry
                    WdbgConditional(ScreensaverDebug, "I", "Drawn all columns. Waiting {0} ms...", LinotypoNewScreenDelay)
                    Console.WriteLine()
                ...
                Else
                    ...
                    WdbgConditional(ScreensaverDebug, "I", "New column. Moving to {0}...", CurrentColumnRowConsole)
                End If
            End If
        ElseIf LinotypoTextColumns = 1 And Console.CursorTop >= Console.WindowHeight - 2 Then
            'We're on the bottom, so wait until retry...
            WdbgConditional(ScreensaverDebug, "I", "Drawn all text. Waiting {0} ms...", LinotypoNewScreenDelay)
            Console.WriteLine()
            SleepNoBlock(LinotypoNewScreenDelay, Linotypo)
