        ...
        Public Overrides Sub Execute(StringArgs As String, ListArgs() As String, ListArgsOnly As String(), ListSwitchesOnly As String()) Implements ICommand.Execute
            Try
                ...
                For Each Match As String In Matches
                    Write(Match, True, GetConsoleColor(ColTypes.Neutral))
                Next
            Catch ex As Exception
                Wdbg(DebugLevel.E, "Error trying to search {0} for {1}", ListArgs(0), ListArgs(1))
                WStkTrc(ex)
                Write(DoTranslation("Searching {0} for {1} failed.") + " {2}", True, color:=GetConsoleColor(ColTypes.Error), ListArgs(0), ListArgs(1), ex.Message)
            End Try
        End Sub
