            ...
            If ListArgsOnly.Length = 2 Then
                If IsStringNumeric(ListArgsOnly(1)) Then
                    If CInt(ListArgsOnly(1)) <= TextEdit_FileLines.Count Then
                        Dim QueriedChars As Dictionary(Of Integer, String) = TextEdit_QueryChar(ListArgsOnly(0), ListArgsOnly(1))
                        For Each CharIndex As Integer In QueriedChars.Keys
                            Write("- {0}: ", False, ColTypes.ListEntry, CharIndex)
                            Write("{0} ({1})", True, ColTypes.ListValue, ListArgsOnly(0), QueriedChars(CharIndex))
                        Next
                    Else
                ...
                ElseIf ListArgsOnly(1).ToLower = "all" Then
                    Dim QueriedChars As Dictionary(Of Integer, Dictionary(Of Integer, String)) = TextEdit_QueryChar(ListArgsOnly(0))
                ...
                If IsStringNumeric(ListArgsOnly(1)) And IsStringNumeric(ListArgsOnly(2)) Then
                    If CInt(ListArgsOnly(1)) <= TextEdit_FileLines.Count And CInt(ListArgsOnly(2)) <= TextEdit_FileLines.Count Then
                        ...
                        For LineNumber = LineNumberStart To LineNumberEnd
                            Dim QueriedChars As Dictionary(Of Integer, String) = TextEdit_QueryChar(ListArgsOnly(0), LineNumber)
                            For Each CharIndex As Integer In QueriedChars.Keys
