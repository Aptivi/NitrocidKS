...
namespace KS.Shell.Shells.UESH
{
    public static class UESHShellCommon
    {
        ...
        public readonly static Dictionary<string, CommandInfo> Commands = new()
        {
            ...
            { "beep", new CommandInfo("beep", ShellType.Shell, "Beep in 'n' Hz and time in 'n' milliseconds", new CommandArgumentInfo(), new BeepCommand()) },
            ...
            { "cat", new CommandInfo("cat", ShellType.Shell, "Prints content of file to console", new CommandArgumentInfo(new[] { "[-lines|-nolines] <file>" }, true, 1), new CatCommand()) },
            ...
            { "dict", new CommandInfo("dict", ShellType.Shell, "The English Dictionary", new CommandArgumentInfo(new[] { "<word>" }, true, 1), new DictCommand()) },
            { "dirinfo", new CommandInfo("dirinfo", ShellType.Shell, "Provides information about a directory", new CommandArgumentInfo(new[] { "<directory>" }, true, 1), new DirInfoCommand()) },
            ...
            { "echo", new CommandInfo("echo", ShellType.Shell, "Writes text into the console", new CommandArgumentInfo(new[] { "[text]" }, false, 0), new EchoCommand()) },
            { "edit", new CommandInfo("edit", ShellType.Shell, "Edits a text file", new CommandArgumentInfo(new[] { "<file>" }, true, 1), new EditCommand()) },
            { "fileinfo", new CommandInfo("fileinfo", ShellType.Shell, "Provides information about a file", new CommandArgumentInfo(new[] { "<file>" }, true, 1), new FileInfoCommand()) },
            ...
            { "genname", new CommandInfo("genname", ShellType.Shell, "Name and surname generator", new CommandArgumentInfo(new[] { "[namescount] [nameprefix] [namesuffix] [surnameprefix] [surnamesuffix]" }, false, 0), new GenNameCommand()) },
            { "gettimeinfo", new CommandInfo("gettimeinfo", ShellType.Shell, "Gets the date and time information", new CommandArgumentInfo(new[] { "<date>" }, true, 1), new GetTimeInfoCommand()) },
            ...
            { "hexedit", new CommandInfo("hexedit", ShellType.Shell, "Edits a binary file", new CommandArgumentInfo(new[] { "<file>" }, true, 1), new HexEditCommand()) },
            { "http", new CommandInfo("http", ShellType.Shell, "Starts the HTTP shell", new CommandArgumentInfo(), new HttpCommand()) },
            { "hwinfo", new CommandInfo("hwinfo", ShellType.Shell, "Prints hardware information", new CommandArgumentInfo(new[] { "<HardwareType>" }, true, 1), new HwInfoCommand()) },
            ...
            { "jsonbeautify", new CommandInfo("jsonbeautify", ShellType.Shell, "Beautifies the JSON file", new CommandArgumentInfo(new[] { "<jsonfile> [output]" }, true, 1), new JsonBeautifyCommand()) },
            { "jsonminify", new CommandInfo("jsonminify", ShellType.Shell, "Minifies the JSON file", new CommandArgumentInfo(new[] { "<jsonfile> [output]" }, true, 1), new JsonMinifyCommand()) },
            { "jsonshell", new CommandInfo("jsonshell", ShellType.Shell, "Opens the JSON shell", new CommandArgumentInfo(new[] { "<jsonfile>" }, true, 1), new JsonShellCommand()) },
            { "keyinfo", new CommandInfo("keyinfo", ShellType.Shell, "Gets key information for a pressed key. Useful for debugging", new CommandArgumentInfo(new[] { "" }, false, 0), new KeyInfoCommand()) },
            ...
            { "list", new CommandInfo("list", ShellType.Shell, "List file/folder contents in current folder", new CommandArgumentInfo(new[] { "[-showdetails|-suppressmessages] [directory]" }, false, 0), new ListCommand()) },
            { "listunits", new CommandInfo("listunits", ShellType.Shell, "Lists all available units", new CommandArgumentInfo(new[] { "<type>" }, true, 1), new ListUnitsCommand()) },
            ...
            { "lsdbgdev", new CommandInfo("lsdbgdev", ShellType.Shell, "Lists debugging devices connected", new CommandArgumentInfo(), new LsDbgDevCommand(), CommandFlags.Strict) },
            { "lsvars", new CommandInfo("lsvars", ShellType.Shell, "Lists available UESH variables", new CommandArgumentInfo(), new LsVarsCommand()) },
            ...
            { "rarshell", new CommandInfo("rarshell", ShellType.Shell, "The RAR shell", new CommandArgumentInfo(new[] { "<rarfile>" }, true, 1), new RarShellCommand()) },
            ...
            { "search", new CommandInfo("search", ShellType.Shell, "Searches for specified string in the provided file using regular expressions", new CommandArgumentInfo(new[] { "<Regexp> <File>" }, true, 2), new SearchCommand()) },
            { "searchword", new CommandInfo("searchword", ShellType.Shell, "Searches for specified string in the provided file", new CommandArgumentInfo(new[] { "<StringEnclosedInDoubleQuotes> <File>" }, true, 2), new SearchWordCommand()) },
            ...
            { "setthemes", new CommandInfo("setthemes", ShellType.Shell, "Sets up kernel themes", new CommandArgumentInfo(new[] { "<Theme>" }, true, 1), new SetThemesCommand()) },
            ...
            { "showtdzone", new CommandInfo("showtdzone", ShellType.Shell, "Shows date and time in zones", new CommandArgumentInfo(new[] { "[-all] <timezone>" }, true, 1), new ShowTdZoneCommand()) },
            ...
            { "weather", new CommandInfo("weather", ShellType.Shell, "Shows weather info for specified city. Uses OpenWeatherMap.", new CommandArgumentInfo(new[] { "[-list] <CityID/CityName> [apikey]" }, true, 1), new WeatherCommand(), CommandFlags.SettingVariable) },
            { "wrap", new CommandInfo("wrap", ShellType.Shell, "Wraps the console output", new CommandArgumentInfo(new[] { "<command>" }, true, 1), new WrapCommand(), CommandFlags.SettingVariable) },
            ...
            { "zipshell", new CommandInfo("zipshell", ShellType.Shell, "Opens a ZIP archive", new CommandArgumentInfo(new[] { "<zipfile>" }, true, 1), new ZipShellCommand()) }
        };
    }
}
