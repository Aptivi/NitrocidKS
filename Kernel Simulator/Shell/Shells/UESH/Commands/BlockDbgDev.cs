    ...
    class BlockDbgDevCommand : CommandExecutor, ICommand
    {
        ...
        public override void Execute(string StringArgs, string[] ListArgsOnly, string[] ListSwitchesOnly)
        {
            if (!RemoteDebugger.RDebugBlocked.Contains(ListArgsOnly[0]))
            {
                ...
                {
                    TextWriterColor.Write(Translate.DoTranslation("{0} can't join remote debug now."), true, ColorTools.ColTypes.Neutral, ListArgsOnly[0]);
                }
                else
                {
                    TextWriterColor.Write(Translate.DoTranslation("Failed to block {0}."), true, ColorTools.ColTypes.Neutral, ListArgsOnly[0]);
                }
            }
            else
            {
                TextWriterColor.Write(Translate.DoTranslation("{0} is already blocked."), true, ColorTools.ColTypes.Neutral, ListArgsOnly[0]);
            }
        }
