...
Namespace Kernel
    Public Module KernelTools
            ...
            Try
                ...
                If [Enum].IsDefined(GetType(KernelErrorLevel), ErrorType) Then
                    If ErrorType = KernelErrorLevel.U Or ErrorType = KernelErrorLevel.D Then
                        ...
                        ElseIf Not Reboot Then
                            ...
                            Write(DoTranslation("[{0}] panic: Reboot enabled due to error level being {0}."), True, color:=GetConsoleColor(ColTypes.Uncontinuable), ErrorType)
                            Reboot = True
                        End If
                    End If
                    If RebootTime > 3600 Then
                        ...
                        Write(DoTranslation("[{0}] panic: Time to reboot: {1} seconds, exceeds 1 hour. It is set to 1 minute."), True, color:=GetConsoleColor(ColTypes.Uncontinuable), ErrorType, CStr(RebootTime))
                        RebootTime = 60
                    End If
                Else
                ...
                Select Case ErrorType
                    Case KernelErrorLevel.D
                        ...
                        Write(DoTranslation("[{0}] dpanic: {1} -- Rebooting in {2} seconds..."), True, color:=GetConsoleColor(ColTypes.Uncontinuable), ErrorType, Description, CStr(RebootTime))
                    ...
                    Case KernelErrorLevel.C
                        If Reboot Then
                            ...
                            Write(DoTranslation("[{0}] panic: Reboot disabled due to error level being {0}."), True, color:=GetConsoleColor(ColTypes.Warning), ErrorType)
                        End If
                        'Print normally
                    ...
                    Case Else
                        If Reboot Then
                            ...
                            Write(DoTranslation("[{0}] panic: {1} -- Rebooting in {2} seconds..."), True, color:=GetConsoleColor(ColTypes.Uncontinuable), ErrorType, Description, CStr(RebootTime))
        ...
        Sub GeneratePanicDump(Description As String, ErrorType As KernelErrorLevel, Exc As Exception)
            ...
            Catch ex As Exception
                Write(DoTranslation("Dump information gatherer crashed when trying to get information about {0}: {1}"), True, color:=GetConsoleColor(ColTypes.Error), Exc.GetType.FullName, ex.Message)
                WStkTrc(ex)
            End Try
        End Sub
        ...
        Sub InitEverything(Args() As String)
            ...
            If ShowAppInfoOnBoot And Not EnableSplash Then
                ...
                Write("OS: " + DoTranslation("Running on {0}"), True, color:=GetConsoleColor(ColTypes.Neutral), Environment.OSVersion.ToString)
            End If
