    ...
    Public Module MailTransfer
        ...
        Public Sub MailPrintMessage(MessageNum As Integer, Optional Decrypt As Boolean = False)
            ...
            If Message < 0 Then
                ...
                Write(DoTranslation("Message number may not be negative or zero."), True, GetConsoleColor(ColTypes.Error))
                Exit Sub
            ElseIf Message > MaxMessagesIndex Then
                ...
                Write(DoTranslation("Message specified is not found."), True, GetConsoleColor(ColTypes.Error))
                Exit Sub
            End If
            ...
            SyncLock IMAP_Client.SyncRoot
                ...
                For Each Address As InternetAddress In Msg.From
                    ...
                    Write(DoTranslation("- From {0}"), True, color:=GetConsoleColor(ColTypes.ListEntry), Address.ToString)
                Next
                ...
                For Each Address As InternetAddress In Msg.To
                    Wdbg(DebugLevel.I, "Address: {0} ({1})", Address.Name, Address.Encoding.EncodingName)
                    Write(DoTranslation("- To {0}"), True, color:=GetConsoleColor(ColTypes.ListEntry), Address.ToString)
                Next
                        ...
                        If MultiEntity IsNot Nothing Then
                            For EntityNumber As Integer = 0 To MultiEntity.Count - 1
                                ...
                                If Not MultiEntity(EntityNumber).IsAttachment Then
                                    ...
                                    Write(Encoding.Default.GetString(DecryptedByte), True, GetConsoleColor(ColTypes.ListValue))
                                End If
                            Next
                        End If
                    Else
...
#Disable Warning BC42104
                If Msg.Attachments.Count > 0 Then
                    Write(DoTranslation("Attachments:"), True, GetConsoleColor(ColTypes.Neutral))
                    ...
                    For Each Attachment As MimeEntity In AttachmentEntities
                        ...
                        If TypeOf Attachment Is MessagePart Then
                            ...
                            Write($"- {Attachment.ContentDisposition?.FileName}", True, GetConsoleColor(ColTypes.Neutral))
                        Else
                            Wdbg(DebugLevel.I, "Attachment is a file.")
                            Dim AttachmentPart As MimePart = Attachment
                            Write($"- {AttachmentPart.FileName}", True, GetConsoleColor(ColTypes.Neutral))
                        End If
                    Next
                End If
