    ...
    Public Module FTPTransferProgress
        ...
        Private Sub FileProgressHandler(Percentage As FtpProgress)
            ...
            Else
                ...
                If progressFlag = True And Percentage.Progress <> 100 Then
                    TextWriterColor.Write(" {0}% (ETA: {1}d {2}:{3}:{4} @ {5})", False, ColTypes.Progress, Percentage.Progress.ToString("N2"), Percentage.ETA.Days, Percentage.ETA.Hours, Percentage.ETA.Minutes, Percentage.ETA.Seconds, Percentage.TransferSpeedToString)
                    ClearLineToRight()
                End If
        ...
        Private Sub MultipleProgressHandler(Percentage As FtpProgress)
            ...
            Else
                ...
                ConsoleOriginalPosition_TOP = Console.CursorTop
                If progressFlag = True And Percentage.Progress <> 100 Then
                    TextWriterColor.Write("- [{0}/{1}] {2}: ", False, ColTypes.ListEntry, Percentage.FileIndex + 1, Percentage.FileCount, Percentage.RemotePath)
                    TextWriterColor.Write("{0}% (ETA: {1}d {2}:{3}:{4} @ {5})", False, ColTypes.Progress, Percentage.Progress.ToString("N2"), Percentage.ETA.Days, Percentage.ETA.Hours, Percentage.ETA.Minutes, Percentage.ETA.Seconds, Percentage.TransferSpeedToString)
                    ClearLineToRight()
                End If
                Console.SetCursorPosition(ConsoleOriginalPosition_LEFT, ConsoleOriginalPosition_TOP)
