...
namespace KS.Login
{
    public static class PermissionManagement
    {
        ...
        {
            { "root", PermissionType.Administrator }
        };
        ...
        public static void AddPermission(PermissionType PermType, string Username)
        {
            ...
            // Save changes
            foreach (JObject UserToken in UserManagement.UsersToken.Cast<JObject>())
            {
        ...
        public static void LoadPermissions()
        {
            foreach (JObject UserToken in UserManagement.UsersToken.Cast<JObject>())
            {
                ...
                foreach (string Perm in ((JArray)UserToken["permissions"]).Select(v => (string)v))
                {
                    switch (Perm ?? "")
                    {
                        case "Administrator":
