            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (maes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    if (idName == nameof(string.Split))
                    {
                        ...
                        TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses .Split(\"\\n\") instead of TextTools.SplitNewLines()", true, ConsoleColors.Yellow);
