                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Directory))
                    {
                        ...
                        if (idName == nameof(Directory.GetFileSystemEntries))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Directory.GetFileSystemEntries instead of Listing.CreateList()", true, ConsoleColors.Yellow);
