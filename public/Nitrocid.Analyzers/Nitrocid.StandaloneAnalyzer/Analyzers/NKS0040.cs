                        ...
                        if (argSyntax.Expression is IdentifierNameSyntax argIdentifier)
                        {
                            if (argIdentifier.Identifier.Text == nameof(OSPlatform))
                            {
                                ...
                                if (idName == nameof(OSPlatform.Linux))
                                {
                                    ...
                                    TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses RuntimeInformation.IsOSPlatform(OSPlatform.Linux) instead of KernelPlatform.IsOnUnix()", true, ConsoleColors.Yellow);
