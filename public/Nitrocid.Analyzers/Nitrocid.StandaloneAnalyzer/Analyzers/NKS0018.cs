                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(File))
                    {
                        ...
                        if (idName == nameof(File.Exists))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses File.Exists instead of Checking.FileExists()", true, ConsoleColors.Yellow);
