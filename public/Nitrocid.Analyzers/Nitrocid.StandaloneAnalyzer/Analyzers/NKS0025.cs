                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == "TimeDateTools")
                    {
                        ...
                        if (idName == "KernelDateTime")
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses TimeDateTools.KernelDateTime.ToString instead of TimeDateRenderers.RenderTime()", true, ConsoleColors.Yellow);
