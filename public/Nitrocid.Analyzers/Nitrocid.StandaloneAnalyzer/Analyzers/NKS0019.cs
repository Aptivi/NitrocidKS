                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Directory))
                    {
                        ...
                        if (idName == nameof(Directory.Exists))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Directory.Exists instead of Checking.FolderExists()", true, ConsoleColors.Yellow);
