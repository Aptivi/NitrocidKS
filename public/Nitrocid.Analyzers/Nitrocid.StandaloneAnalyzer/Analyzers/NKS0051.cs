                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(NetworkInterface))
                    {
                        ...
                        if (idName == nameof(NetworkInterface.GetIsNetworkAvailable))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses NetworkInterface.GetIsNetworkAvailable instead of NetworkTools.NetworkAvailable", true, ConsoleColors.Yellow);
