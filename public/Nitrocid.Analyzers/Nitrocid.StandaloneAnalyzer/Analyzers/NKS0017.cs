                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Directory))
                    {
                        ...
                        if (idName == nameof(Directory.Delete))
                        {
                            ...
                            TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Directory.Delete instead of Removing.RemoveDirectory()", true, ConsoleColors.Yellow);
