                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Path))
                    {
                        ...
                        if (idName == nameof(Path.GetInvalidPathChars))
                        {
                            ...
                            TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Path.GetInvalidPathChars instead of Parsing.GetInvalidPathChars()", true, ConsoleColors.Yellow);
