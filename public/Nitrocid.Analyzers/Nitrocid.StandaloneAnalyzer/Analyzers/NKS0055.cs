            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (maes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    if (idName == "SplitNewLinesOld")
                    {
                        ...
                        TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses TextTools.SplitNewLinesOld() instead of TextTools.SplitNewLines()", true, ConsoleColors.Yellow);
