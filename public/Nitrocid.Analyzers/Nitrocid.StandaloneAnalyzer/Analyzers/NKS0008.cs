                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Console))
                    {
                        ...
                        if (idName == nameof(Console.WriteLine))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Console.WriteLine instead of TWC.Write()", true, ConsoleColors.Yellow);
