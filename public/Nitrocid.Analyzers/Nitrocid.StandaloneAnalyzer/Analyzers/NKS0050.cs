            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (expMaes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    if (idName == nameof(string.Split))
                    {
                        ...
                        TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses CharManager.NewLine instead of TextTools.SplitNewLines()", true, ConsoleColors.Yellow);
