                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(File))
                    {
                        ...
                        if (idName == nameof(File.Delete))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses File.Delete instead of Removing.RemoveFile()", true, ConsoleColors.Yellow);
