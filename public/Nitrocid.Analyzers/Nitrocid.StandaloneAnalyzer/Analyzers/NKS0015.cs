                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(File))
                    {
                        ...
                        if (idName == nameof(File.Move))
                        {
                            ...
                            TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses File.Move instead of Moving.MoveFileOrDir()", true, ConsoleColors.Yellow);
