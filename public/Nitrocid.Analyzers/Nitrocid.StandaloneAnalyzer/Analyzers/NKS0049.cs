            ...
            foreach (var syntaxNode in syntaxNodeNodes)
            {
                ...
                if (expMaes.Name is IdentifierNameSyntax identifier)
                {
                    ...
                    if (idName == nameof(string.Split))
                    {
                        ...
                        TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Environment.NewLine instead of TextTools.SplitNewLines()", true, ConsoleColors.Yellow);
