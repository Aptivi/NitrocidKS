                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(TimeZoneInfo))
                    {
                        ...
                        if (idName == nameof(TimeZoneInfo.Local))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses TimeZoneInfo.Local instead of TimeZones.GetCurrentZoneInfo()", true, ConsoleColors.Yellow);
