                ...
                if (exp.Expression is MemberAccessExpressionSyntax maes)
                {
                        ...
                        identifierRight.Identifier.Text == "CurrentUICulture")
                    {
                        ...
                        if (idName == "Name")
                        {
                            ...
                            TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses CultureInfo.CurrentUICulture.Name instead of CultureManager.CurrentCultStr", true, ConsoleColors.Yellow);
