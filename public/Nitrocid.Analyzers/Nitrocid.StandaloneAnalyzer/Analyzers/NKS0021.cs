                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Path))
                    {
                        ...
                        if (idName == nameof(Path.IsPathRooted))
                        {
                            ...
                            TextWriterColor.Write($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Path.IsPathRooted instead of Checking.Rooted()", true, ConsoleColors.Yellow);
