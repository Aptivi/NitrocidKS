                ...
                if (exp.Expression is IdentifierNameSyntax identifier)
                {
                    ...
                    if (identifier.Identifier.Text == nameof(Console))
                    {
                        ...
                        if (idName == nameof(Console.ReadLine))
                        {
                            ...
                            TextWriterColor.WriteColor($"{GetType().Name}: {document.FilePath} ({lineSpan.StartLinePosition} -> {lineSpan.EndLinePosition}): Caller uses Console.ReadLine instead of ReadLine()", true, ConsoleColors.Yellow);
