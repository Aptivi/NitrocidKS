...
namespace KS.Shell.Shells.UESH
{
    ...
    internal class UESHShellInfo : BaseShellInfo, IShellInfo
    {
        ...
        public override Dictionary<string, CommandInfo> Commands => new()
        {
            ...
            { "alias", new CommandInfo("alias", ShellType, "Adds aliases to commands", new CommandArgumentInfo( new[] { $"<rem/add> <{string.Join("/", Enum.GetNames(typeof(ShellType)))}> <alias> <cmd>" }, true, 3, (startFrom, _, _) => HelpUnifiedCommand.ListCmds(startFrom)), new AliasCommand(), CommandFlags.Strict) },
            ...
            { "chpwd", new CommandInfo("chpwd", ShellType, "Changes password for current user", new CommandArgumentInfo(new[] { "<Username> <UserPass> <newPass> <confirm>" }, true, 4, (startFrom, _, _) => UserManagement.ListAllUsers().Where((src) => src.StartsWith(startFrom)).ToArray()), new ChPwdCommand(), CommandFlags.Strict) },
            { "chusrname", new CommandInfo("chusrname", ShellType, "Changes user name", new CommandArgumentInfo(new[] { "<oldUserName> <newUserName>" }, true, 2, (startFrom, _, _) => UserManagement.ListAllUsers().Where((src) => src.StartsWith(startFrom)).ToArray()), new ChUsrNameCommand(), CommandFlags.Strict) },
            ...
            { "hwinfo", new CommandInfo("hwinfo", ShellType, "Prints hardware information", new CommandArgumentInfo(new[] { "<HardwareType>" }, true, 1, (_, _, _) => new[] { "HDD", "LogicalParts", "CPU", "GPU", "Sound", "Network", "System", "Machine", "BIOS", "RAM", "all" }), new HwInfoCommand(), CommandFlags.RedirectionSupported | CommandFlags.Wrappable) },
            ...
            { "langman", new CommandInfo("langman", ShellType, "Manage your languages", new CommandArgumentInfo(new[] { "<reload/load/unload> <customlanguagename>", "<list/reloadall>" }, true, 1, (startFrom, _, _) => Languages.LanguageManager.CustomLanguages.Keys.Where((src) => src.StartsWith(startFrom)).ToArray()), new LangManCommand(), CommandFlags.Strict) },
            ...
            { "listunits", new CommandInfo("listunits", ShellType, "Lists all available units", new CommandArgumentInfo(new[] { "<type>" }, true, 1, (startFrom, _, _) => Quantity.Infos.Select((src) => src.Name).Where((src) => src.StartsWith(startFrom)).ToArray()), new ListUnitsCommand(), CommandFlags.RedirectionSupported | CommandFlags.Wrappable) },
            ...
            { "perm", new CommandInfo("perm", ShellType, "Manage permissions for users", new CommandArgumentInfo(new[] { "<userName> <allow/revoke> <perm>" }, true, 3, (startFrom, _, _) => UserManagement.ListAllUsers().Where((src) => src.StartsWith(startFrom)).ToArray()), new PermCommand(), CommandFlags.Strict) },
            ...
            { "previewsplash", new CommandInfo("previewsplash", ShellType, "Previews the splash", new CommandArgumentInfo(new[] { "[splashName]" }, false, 0, (startFrom, _, _) => SplashManager.Splashes.Keys.Where((src) => src.StartsWith(startFrom)).ToArray()), new PreviewSplashCommand()) },
            ...
            { "rmuser", new CommandInfo("rmuser", ShellType, "Removes a user from the list", new CommandArgumentInfo(new[] { "<Username>" }, true, 1, (startFrom, _, _) => UserManagement.ListAllUsers().Where((src) => src.StartsWith(startFrom)).ToArray()), new RmUserCommand(), CommandFlags.Strict) },
            ...
            { "savescreen", new CommandInfo("savescreen", ShellType, "Saves your screen from burn outs", new CommandArgumentInfo(new[] { "[saver]" }, false, 0, (startFrom, _, _) => Screensaver.Screensavers.Keys.Where((src) => src.StartsWith(startFrom)).ToArray()), new SaveScreenCommand()) },
            ...
            { "themesel", new CommandInfo("themesel", ShellType, "Selects a theme and sets it", new CommandArgumentInfo(new[] { "[Theme]" }, false, 0, (startFrom, _, _) => ThemeTools.Themes.Keys.Where((src) => src.StartsWith(startFrom)).ToArray()), new ThemeSelCommand()) },
