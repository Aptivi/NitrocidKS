                                       ...
                                       KernelColorType InfoBoxTitledButtonsColor, KernelColorType BackgroundColor, params object[] vars)
        {
            ...
            if (buttons.Length > 3)
            {
                ...
                for (int i = 1; i <= buttons.Length; i++)
                    buttonNums.Add($"{i}");
                var choices = InputChoiceTools.GetInputChoices(buttonNums.ToArray(), buttons).ToArray();
                return InfoBoxTitledSelectionColor.WriteInfoBoxTitledSelectionKernelColor(title, choices, text, InfoBoxTitledButtonsColor, BackgroundColor);
            }
            ...
            try
            {
                ...
                string border = BorderTextColor.RenderBorderTextPlain(title, borderX, borderY, maxWidth, maxHeight, UpperLeftCornerChar, LowerLeftCornerChar, UpperRightCornerChar, LowerRightCornerChar, UpperFrameChar, LowerFrameChar, LeftFrameChar, RightFrameChar);
                boxBuffer.Append(
                    $"{KernelColorTools.GetColor(InfoBoxTitledButtonsColor).VTSequenceForeground}" +
                ...
                while (!bail)
                {
                    ...
                    for (int i = 1; i <= buttons.Length; i++)
                    {
                        ...
                        // Render the button box
                        input.Append(
                            BorderTextColor.RenderBorderText(title, buttonX, buttonPanelPosY, maxButtonWidth - 3, 1, KernelColorTools.GetColor(buttonForegroundColor), KernelColorTools.GetColor(buttonBackgroundColor)) +
