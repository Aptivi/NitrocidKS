...
{
    class HTTP_PostCommand : BaseCommand, ICommand
    {

        public override async void Execute(string StringArgs, string[] ListArgsOnly, string[] ListSwitchesOnly)
        {
            // Print a message
            TextWriterColor.Write(Translate.DoTranslation("Posting file {0}..."), true, KernelColorType.Progress, ListArgsOnly[0]);

            try
            {
                var ResponseTask = HTTPTools.HttpPostFile(ListArgsOnly[0], ListArgsOnly[1]);
                ResponseTask.Wait();
