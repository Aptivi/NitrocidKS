...
namespace KS.Misc.Interactives
{
    ...
    /// </summary>
    public class FilesSelectorCli : BaseInteractiveTui, IInteractiveTui
    {
        ...
        [
            // Operations
            new InteractiveTuiBinding(/* Localizable */ "Select", ConsoleKey.Enter,
                (info, _) => SelectOrGoTo((FileSystemEntry)info), true),
        ...
        private static void SelectOrGoTo(FileSystemEntry currentFileSystemEntry)
        {
            try
            {
                ...
                else if (currentFileSystemEntry.Type == FileSystemEntryType.File)
                {
                    ...
                    if (!((FilesSelectorCli)Instance).selectedFiles.Remove(currentFileSystemEntry.FilePath))
                    {
                        ((FilesSelectorCli)Instance).selectedFiles.Add(currentFileSystemEntry.FilePath);
                        InfoBoxColor.WriteInfoBoxColorBack(Translate.DoTranslation("Selected '{0}'. Press ESC to exit and confirm selections."), BoxForegroundColor, BoxBackgroundColor, currentFileSystemEntry.FilePath);
                    }
                    else
                        InfoBoxColor.WriteInfoBoxColorBack(Translate.DoTranslation("Unselected '{0}'. Press ESC to exit and confirm selections."), BoxForegroundColor, BoxBackgroundColor, currentFileSystemEntry.FilePath);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void GoTo()
        {
            ...
            path = FilesystemTools.NeutralizePath(path, ((FilesSelectorCli)Instance).firstPanePath);
