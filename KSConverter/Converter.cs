    ...
    static class Converter
    {
        ...
        public static void Main()
        {
            ...
            try
            {
                ...
                SeparatorWriterColor.WriteSeparator("[1/7] Making backup directory", true, ColorTools.ColTypes.Stage);
                ...
                {
                    // Just make it!
                    TextWriterColor.Write("  - Backup directory not found. Creating directory...", true, ColorTools.ColTypes.Progress);
                ...
                else
                {
                    ...
                    TextWriterColor.Write("  - Warning: backup directory is already found.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Make backup of old configuration files in case something goes wrong during conversion.
                SeparatorWriterColor.WriteSeparator("[2/7] Making backup of old configuration files", true, ColorTools.ColTypes.Stage);
                foreach (string ConfigEntry in ListOfOldPaths.Keys)
                {
                    ...
                    if (Checking.FileExists(ListOfOldPaths[ConfigEntry]) & !Checking.FileExists(ListOfBackups[ConfigEntry]))
                    {
                        ...
                        TextWriterColor.Write("  - {0}: {1} -> {2}", true, ColorTools.ColTypes.Neutral, ConfigEntry, ListOfOldPaths[ConfigEntry], ListOfBackups[ConfigEntry]);
                    ...
                    else if (Checking.FileExists(ListOfBackups[ConfigEntry]))
                    {
                        ...
                        TextWriterColor.Write("  - Warning: {0} already exists", true, ColorTools.ColTypes.Warning, ListOfBackups[ConfigEntry]);
                    }
                    else
                    {
                        // File not found. Skip it.
                        Debug.WriteLine("We don't have config.");
                        TextWriterColor.Write("  - Warning: {0} not found in home directory.", true, ColorTools.ColTypes.Warning, ListOfOldPaths[ConfigEntry]);
                    }
                }
                Console.WriteLine();

                // Import all blocked devices to DebugDeviceNames.json
                SeparatorWriterColor.WriteSeparator("[3/7] Importing all blocked devices to DebugDeviceNames.json", true, ColorTools.ColTypes.Stage);
                ...
                if (Checking.FileExists(ListOfBackups["BlockedDevices"]))
                {
                    ...
                    TextWriterColor.Write("  - Reading blocked devices from blocked_devices.csv...", true, ColorTools.ColTypes.Progress);
                    ...
                    TextWriterColor.Write("  - {0} devices found.", true, ColorTools.ColTypes.Neutral, BlockedDevices.Count);
                    ...
                    foreach (string BlockedDevice in BlockedDevices)
                    {
                        ...
                        TextWriterColor.Write("  - Adding {0} to DebugDeviceNames.json...", true, ColorTools.ColTypes.Progress, BlockedDevice);
                ...
                else
                {
                    ...
                    TextWriterColor.Write("  - Warning: blocked_devices.csv not found in home directory.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Import all FTP speed dial settings to JSON
                SeparatorWriterColor.WriteSeparator("[4/7] Importing all FTP speed dial addresses to FTP_SpeedDial.json", true, ColorTools.ColTypes.Stage);
                ...
                if (Checking.FileExists(ListOfBackups["FTPSpeedDial"]))
                {
                    ...
                    TextWriterColor.Write("  - Reading FTP speed dial addresses from ftp_speeddial.csv...", true, ColorTools.ColTypes.Progress);
                    ...
                    TextWriterColor.Write("  - {0} addresses found.", true, ColorTools.ColTypes.Neutral, SpeedDialLines.Length);
                    ...
                    foreach (string SpeedDialLine in SpeedDialLines)
                    {
                        ...
                        // Add the entry!
                        TextWriterColor.Write("  - Adding {0} to FTP_SpeedDial.json...", true, ColorTools.ColTypes.Progress, Address);
                ...
                else
                {
                    // File not found. Skip stage.
                    Debug.WriteLine("We don't have file.");
                    TextWriterColor.Write("  - Warning: ftp_speeddial.csv not found in home directory.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Import all users to JSON
                SeparatorWriterColor.WriteSeparator("[5/7] Importing all users to Users.json", true, ColorTools.ColTypes.Stage);
                ...
                if (Checking.FileExists(ListOfBackups["Users"]))
                {
                    ...
                    TextWriterColor.Write("  - Reading users from users.csv...", true, ColorTools.ColTypes.Progress);
                    ...
                    TextWriterColor.Write("  - {0} users found.", true, ColorTools.ColTypes.Neutral, UsersLines.Length);
                    ...
                    foreach (string UsersLine in UsersLines)
                    {
                        ...
                        // Add the entry!
                        TextWriterColor.Write("  - Adding {0} to Users.json...", true, ColorTools.ColTypes.Progress, Username);
                ...
                else
                {
                    // File not found. Skip stage.
                    Debug.WriteLine("We don't have file.");
                    TextWriterColor.Write("  - Warning: users.csv not found in home directory.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Import all aliases to JSON
                SeparatorWriterColor.WriteSeparator("[6/7] Importing all aliases to Aliases.json", true, ColorTools.ColTypes.Stage);
                ...
                if (Checking.FileExists(ListOfBackups["Aliases"]))
                {
                    ...
                    TextWriterColor.Write("  - Reading users from aliases.csv...", true, ColorTools.ColTypes.Progress);
                    ...
                    TextWriterColor.Write("  - {0} aliases found.", true, ColorTools.ColTypes.Neutral, AliasesLines.Length);
                    ...
                    foreach (string AliasLine in AliasesLines)
                    {
                        ...
                        // Add the entry!
                        TextWriterColor.Write("  - Adding {0} to Aliases.json...", true, ColorTools.ColTypes.Progress, AliasCommand);
                        switch (AliasType ?? "")
                        {
                            case "Shell":
                            ...
                            case "Mail":
                                ...
                                break;
                            default:
                                TextWriterColor.Write("  - Invalid type {0}", true, ColorTools.ColTypes.Error, AliasType);
                                break;
                        }
                    }
                    ...
                    TextWriterColor.Write("  - Saving aliases to Aliases.json...", true, ColorTools.ColTypes.Progress);
                ...
                else
                {
                    // File not found. Skip stage.
                    Debug.WriteLine("We don't have file.");
                    TextWriterColor.Write("  - Warning: aliases.csv not found in home directory.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Import all config to JSON
                SeparatorWriterColor.WriteSeparator("[7/7] Importing all kernel config to KernelConfig.json", true, ColorTools.ColTypes.Stage);
                ...
                else
                {
                    // File not found. Skip stage.
                    Debug.WriteLine("We don't have file.");
                    TextWriterColor.Write("  - Warning: kernelConfig.ini not found in home directory.", true, ColorTools.ColTypes.Warning);
                }
                Console.WriteLine();

                // Print this message:
                TextWriterColor.Write("- Successfully converted all settings to new format! Enjoy!", true, ColorTools.ColTypes.Success);
                TextWriterColor.Write("- Press any key to exit.", true, ColorTools.ColTypes.Success);
                Console.ReadKey(true);
            }
            catch (Exception ex)
            {
                TextWriterColor.Write("- Error converting settings: {0}", true, ColorTools.ColTypes.Error, ex.Message);
                TextWriterColor.Write("- Press any key to exit. Stack trace below:", true, ColorTools.ColTypes.Error);
                TextWriterColor.Write(ex.StackTrace, true, ColorTools.ColTypes.Neutral);
                Console.ReadKey(true);
            }
        }
